...
module('Acceptance | home page', function (hooks) {
  ...
  test('displays patch versions for released versions', async function (assert) {
    assert.dom('.version-card:not(.upcoming-version):not(.in-development-version)').exists();
    assert.dom('.version-card:not(.upcoming-version):not(.in-development-version) .patch-versions').exists();
